import {
	earlySlashPublicationCommand,
	pickReviewersCommand,
} from "@packages/cqrs"
...
import { HumanDecisionSchema, type NetworkT } from "@packages/schema"
import { AgentType, getThreadId } from "@packages/utils"
import "server-only"
import { submissionGraph } from "./graph"

const PINATA_IPFS_URL =
	env.NEXT_PUBLIC_PINATA_IPFS_URL || "https://ipfs.io/ipfs"
...
export const startSubmissionAgent = async (params: Params) => {
	...
	try {
		...
		console.log(
			`[Submission Agent] Starting LangGraph for Pub #${publicationId} (Thread: ${threadId})`,
		)

		const finalState = await submissionGraph.invoke(
		...
		console.error(
			`[CRITICAL ERROR] [Submission Agent] crashed for Pub #${publicationId}:`,
			error,
		)
