import { LlmDecisionSchema, LlmVerdictSchema } from "@packages/schema"
...
export const llmFinalVerdictNode = async (state: ReviewsState) => {
	...
	if (!seniorReview?.verdict) {
		throw new Error(
			"Critical Error: Escalation path reached without a Senior Reviewer verdict.",
		)
	}
  ...
  `)
    ...
    - Senior Reviewer Decision: ${seniorReview.verdict.decision}
    - Senior Reviewer Reasoning: ${seniorReview.verdict.reason}
  ...
  `)
	...
	const finalizedDecision = {
		decision: seniorReview.verdict.decision, // Force human-finality
