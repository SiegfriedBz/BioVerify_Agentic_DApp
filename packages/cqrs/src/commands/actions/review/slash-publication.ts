...
export async function slashPublicationCommand(params: Params) {
  const { network, publicationId, rootCid, reason, honestAddresses, negligentAddresses } = params
  ...
  try {
    ...
    const { request } = await publicClient.simulateContract({
      ...
      args: [BigInt(publicationId), honestAddresses, negligentAddresses, verdictCid]
    })
    ...
    console.error(`❌ [CQRS] slashPublicationCommand Failed for Pub #${publicationId}:`, error)
    throw error
  }
}
...
async function notify(params: Params & { verdictCid: string }) {
  const { network, publicationId, rootCid, reason, honestAddresses, negligentAddresses, verdictCid } = params

  const mask = (addr: string) => `\`${addr.slice(0, 6)}...${addr.slice(-4)}\``
  const honestList = honestAddresses.length > 0 ? honestAddresses.map(mask).join(', ') : "_None_"
  const negligentList = negligentAddresses.length > 0 ? negligentAddresses.map(mask).join(', ') : "_None_"
