...
export async function recordReviewCommand(params: Params) {
  ...
  try {
    console.log(`[CQRS] Executing recordReview | Pub: #${publicationId} | Reviewer: ${reviewerAddress}`)
    ...
    console.error(`[CQRS] recordReviewCommand Failed for Pub #${publicationId}:`, error)
    throw error
  }
}