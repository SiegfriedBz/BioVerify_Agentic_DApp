...
async function notify(params: Params & { verdictCid: string }) {
  ...
  const honestList = honestAddresses.map(addr => `\`${addr.slice(0, 6)}...${addr.slice(-4)}\``).join(', ')

  const message = `✅ *BioVerify Alert: Publication Passed Review*\n\n` +
    `*Network:* #${networkMessage(network)}\n` +
    `*Publication:* #${publicationId}\n\n` +
    `🌟 *Honest (Rewarded):*\n${honestList}\n\n` +
    (isPerfect ? `✨ *Perfect Review*` : `⚠️ *Negligent:* ${negligentAddresses.join(', ')}`) +
    `\n\n📝 *Verdict:*\n> ${reason.slice(0, 300)}...\n\n` +
