...
export async function processContractEvent(params: Params) {
	const { chainId, decoded, blockNumber, logIndex, inngest } = params
	...
	switch (eventName) {
		...
		case "Agent_MoveSlashPoolToRewardPool":
			await upsertProtocol(
				chainId,
				{ slashPool: args.newSlashPool, rewardPool: args.newRewardPool },
				blockNumber,
				logIndex,
			)
			break
		...
		case "MemberAvailableStake":
			await upsertMember(
				mId(args.member),
				{
					address: args.member.toLowerCase(),
					chainId,
					availableStake: args.newAvailStake,
				},
				blockNumber,
				logIndex,
			)
		...
		case "MemberLockedStake":
			await upsertMember(
				mId(args.member),
				{
					address: args.member.toLowerCase(),
					chainId,
					lockedStake: args.newLockedStake,
				},
				blockNumber,
				logIndex,
			)
		...
		case "IsAvailableReviewer":
			await upsertMember(
				mId(args.reviewer),
				{
					address: args.reviewer.toLowerCase(),
					chainId,
		...
		case "RewardMember":
			await db
				.update(memberDbSchema)
				.set({
					rewardsCount: sql`${memberDbSchema.rewardsCount} + 1`,
					updatedAt: new Date(),
				})
				.where(
					and(
						eq(memberDbSchema.id, mId(args.member)),
						versionCheck(memberDbSchema, blockNumber, logIndex),
					),
				)
			break
		...
		case "SlashMember":
			await db
				.update(memberDbSchema)
				.set({
					slashesCount: sql`${memberDbSchema.slashesCount} + 1`,
					updatedAt: new Date(),
				})
				.where(
					and(
		...
		case "Agent_PickReviewers": {
			...
			await upsertPublication(
				pId(pubId),
				{ reviewers, seniorReviewer: seniorReviewer.toLowerCase(), status: 2 },
				blockNumber,
				logIndex,
			)
		...
		case "Agent_RecordReview": {
			...
			await db
				...
				.where(
					and(
						eq(publicationDbSchema.id, pId(pubId)),
						versionCheck(publicationDbSchema, blockNumber, logIndex),
					),
				)
			break
		}
		...
		case "Agent_FinalizePublication":
			await upsertPublication(
				pId(args.pubId),
				{ verdictCid: args.verdictCid, status: args.status },
				blockNumber,
				logIndex,
			)
			break
...
async function upsertMember(
	...
	await db
		...
		.onConflictDoUpdate({
			...
			where: versionCheck(memberDbSchema, blockNumber, logIndex),
		})
}
...
async function upsertPublication(
	...
	await db
		...
		.onConflictDoUpdate({
			...
			where: versionCheck(publicationDbSchema, blockNumber, logIndex),
		})
}
...
async function upsertProtocol(
	...
	await db
		...
		.where(
			and(
				eq(protocolDbSchema.id, chainId.toString()),
				versionCheck(protocolDbSchema, blockNumber, logIndex),
			),
		)
}
