...
import { ProtocolConstants, ProtocolConstantsSchema } from "@/app/_schemas/schemas/contract/protocol-constants"
...
export const getProtocolConstants = async (): Promise<ProtocolConstants> => {
  ...
  // TODO RECHECK TYPES OF numWords and minReviewsCount
  const data: ProtocolConstants = {
    ...
    numWords: results[2].status === "success" ? results[2].result as number : 0,
    ...
    minReviewsCount: results[8].status === "success" ? results[8].result as number : 0
  }
