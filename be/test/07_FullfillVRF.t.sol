...
contract FulfillVRFTest is TestHelpers {
    function test_FulfillRandomWords_Success() public {
        ...
        uint256 numOfPeers = VRF_NUM_WORDS - 1;
        ...
        uint256 expectedRewardPool = rewardPoolInit - (VRF_NUM_WORDS * REVIEWER_REWARD);
    ...
    function test_FulfillRandomWords_LoopBypassLogic() public {
        ...
        uint256 expectedRewardPoolDeduction = VRF_NUM_WORDS * REVIEWER_REWARD;
        // The STAKE is what gets locked, not deducted from the pool.
        uint256 expectedTotalLockedStake = VRF_NUM_WORDS * REVIEWER_STAKE;
        ...
        address[] memory lockedReviewers = new address[](VRF_NUM_WORDS);
        ...
        assertEq(uniqueCount, VRF_NUM_WORDS);
    ...
    function test_FulfillRandomWords_RevertIf_PoolShrinksDuringVRF() public {
        ...
        // 4. The reviewer leaves the pool, dropping size below I_VRF_NUM_WORDS + 1
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                VRF_NUM_WORDS // The pool size is now exactly I_VRF_NUM_WORDS, but needs +1
            )
        );
    ...
    function test_FulfillRandomWords_RevertIf_RewardsDepletedDuringVRF() public {
        ...
        for (uint256 i = 0; i < (VRF_NUM_WORDS * 2) + 1; i++) {
        ...
        uint256 rewardsForOnefillment = VRF_NUM_WORDS * REVIEWER_REWARD;

        vm.deal(address(this), rewardsForOnefillment);
        (bool success,) = address(poorContract).call{value: rewardsForOnefillment}("");
        require(success, "Transfer failed");

        // 5. Request reviewers for both (Both pass the reward check >= VRF_NUM_WORDS * REVIEWER_REWARD)
        ...
        uint256[] memory wordsOne = new uint256[](VRF_NUM_WORDS);
        for (uint256 i = 0; i < VRF_NUM_WORDS; ++i) {
            wordsOne[i] = i;
        }
        ...
        // It checks (pool >= VRF_NUM_WORDS * REVIEWER_REWARD). Pool is only 0 REVERT!
        uint256[] memory wordsTwo = new uint256[](VRF_NUM_WORDS);
        for (uint256 i = 0; i < VRF_NUM_WORDS; ++i) {
            wordsTwo[i] = i + VRF_NUM_WORDS;
        }
