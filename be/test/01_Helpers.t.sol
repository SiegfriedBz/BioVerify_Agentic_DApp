...
contract TestHelpers is BaseTest {
    ...
    function _expectEmit_SubmitPublication(address _member, uint256 _pubId, string memory _cid) internal {
        // event SubmitPublication(address indexed publisher, uint256 indexed pubId, string cid);
        vm.expectEmit(true, true, false, true);
        emit SubmitPublication(_member, _pubId, _cid);
    }
    ...
    function _expectEmit_Agent_TransferSlashPoolToTreasury(address _to, uint256 _value) internal {
        // event Agent_TransferSlashPoolToTreasury(address indexed to, uint256 indexed value);
        vm.expectEmit(true, true, false, false);
        emit Agent_TransferSlashPoolToTreasury(_to, _value);
    }

    function _expectEmit_Agent_MoveSlashPoolToRewardPool(uint256 _value) internal {
        // event Agent_MoveSlashPoolToRewardPool(uint256 indexed value);
        vm.expectEmit(true, false, false, false);
        emit Agent_MoveSlashPoolToRewardPool(_value);
    }
    ...
    function _fillValidReviewerPool() internal {
        _fillReviewerPool(VALID_MIN_REVIEWERS_POOL_SIZE);
    }
