...
type Props = {
	stakeWei: bigint | null // From protocol config
	effectiveFeeWei: bigint | null // From custom hook
}

export const FeeCalculator: FC<Props> = ({ stakeWei, effectiveFeeWei }) => {
	const isLoading = stakeWei === null || effectiveFeeWei === null

	const totalWei = !isLoading ? (stakeWei ?? 0n) + (effectiveFeeWei ?? 0n) : 0n
		...
		<Card className="border-primary/20 bg-primary/2 overflow-hidden shadow-sm transition-all">
			<div className="bg-primary/5 px-4 py-2 border-b border-primary/10 flex justify-between items-center h-10">
				...
				<div className="flex items-center gap-1 text-muted-foreground">
					...
					<span className="text-[9px] font-bold uppercase">Fixed stake</span>
				</div>
			</div>
