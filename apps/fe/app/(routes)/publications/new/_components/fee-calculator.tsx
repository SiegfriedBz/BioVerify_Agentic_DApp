...
import { AlertCircleIcon, FuelIcon, LandmarkIcon, WalletIcon } from "lucide-react"
...
export const FeeCalculator: FC<Props> = ({
  ...
  // Logic checks
  const totalWei = !isLoading ? (stakeWei + effectiveFeeWei!) : 0n
  const isInsufficient = !isLoading && stakeWei < minStakeWei!
      ...
      <div className="bg-primary/5 px-4 py-2 border-b border-primary/10 flex justify-between items-center h-10">
        ...
        {isInsufficient && (
          <div className="flex items-center gap-1 text-destructive animate-pulse">
            ...
            <span className="text-[9px] font-bold uppercase">Below Min Stake</span>
          </div>
        )}
      </div>
      ...
      <CardContent className="p-5 space-y-4">
        ...
        <div className="flex justify-between items-start">
          <div className="flex gap-2">
            ...
            <div className="flex flex-col">
              <TypographySmall className="font-medium">Protocol Fee</TypographySmall>
              <TypographySmall className="text-[10px] text-muted-foreground">Covers VRF & Platform</TypographySmall>
            </div>
          </div>
          {isLoading ? (
            <Skeleton className="h-5 w-16 mt-1" />
          ) : (
            <span className="font-mono text-sm">{formatEther(effectiveFeeWei!)} ETH</span>
          )}
        </div>
        ...
        <div className="flex justify-between items-start">
          <div className="flex gap-2">
            ...
            <div className="flex flex-col">
              <TypographySmall className="font-medium">Your Research Stake</TypographySmall>
              <div className="flex items-center gap-1 h-4">
                <TypographySmall className="text-[10px] text-muted-foreground">Minimum required:</TypographySmall>
                {isLoading ? (
          ...
          <span className={`font-mono text-sm ${isInsufficient ? 'text-destructive font-bold' : ''}`}>
