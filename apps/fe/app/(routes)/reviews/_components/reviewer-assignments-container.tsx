...
import { MemberAssignments } from "@packages/cqrs"
...
export const ReviewerAssignmentsContainer: FC<Props> = (props) => {
  ...
  const { data: assignmentsData, isError, refetch } = useMemberAssignments({
    userAddress: activeAddress,
    initialData: activeAddress.toLowerCase() === server.userAddress.toLowerCase() ? server.initialData : undefined
  })
      ...
      {count > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed rounded-2xl bg-muted/5">
          ...
          <p className="text-sm text-muted-foreground/60 max-w-70 text-center mt-1">
            ...
            <span className="inline-flex">New publications will appear here when assigned.</span>
          </p>
        </div>
      )}
    </div>
