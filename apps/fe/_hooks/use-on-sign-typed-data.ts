...
import { HumanDecision } from "@packages/schema"
...
export const useOnSignTypeData = (params: Params) => {
  ...
  const signTd = useCallback((params: SignDataParams) => {
    ...
    if (!publicationId || !rootCid || !walletAddress || !walletChainId || walletChainId !== publication.chainId) {
      return
    }
  ...
  }, [publication, onSuccess, signTypedData, walletChainId])
