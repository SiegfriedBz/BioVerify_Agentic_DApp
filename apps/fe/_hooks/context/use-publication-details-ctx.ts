...
export const PublicationDetailContext =
	createContext<PublicationDetailContextT>({
		publication: null,
		isSyncing: true,
	})
...
export const usePublicationDetailContext = () => {
	...
	if (!ctx) {
		throw new Error(
			"usePublicationDetailContext must be used within its PublicationDetailProvider",
		)
	}
	return ctx
}
