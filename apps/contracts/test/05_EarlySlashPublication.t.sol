...
contract EarlySlashPublicationTest is TestHelpers {
    function test_EarlySlashPublication_Success() public {
        ...
        _expectEmit_SlashPool(publisherStake);
    ...
    function test_EarlySlashPublication_DecreasesHighReputation() public {
        // 1. First success -> Publisher Rep = I_reputationBoost
        _successPublishPublication(0);

        // 2. Second success -> Publisher Rep = 2 * I_reputationBoost
        _successPublishPublication(1);

        // 3. Third success -> Publisher Rep = 3 * I_reputationBoost
        ...
        // currentRep > I_reputationBoost ? currentRep - I_reputationBoost : 0;
        _expectEmit_MemberReputation(publisher, 2 * reputationBoost); // (3 * reputationBoost - 2 * reputationBoost)
